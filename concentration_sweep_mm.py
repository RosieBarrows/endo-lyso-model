"""
v0.3 -- Michaelis-Menten uptake + K_m sweep.

Replaces the linear uptake term (k_uptake * C_ext) of v0.1/v0.2 with the full
saturating form:

    uptake = V_max * C_ext / (K_m + C_ext),   V_max = k_uptake_fitted * K_m

so that at C_ext << K_m the v0.2 calibration is preserved, and at C_ext >> K_m
uptake saturates at V_max. k_deg and all trafficking rates are unchanged from the
v0.2 Step 1 / Step 2A fits.

Sweep K_m in {50, 100, 200, 500} uM over doses {34, 125, 250, 500, 1000, 2000} uM,
24h exposure + 24h washout. Figures -> ./mm_sweep/.
"""
...
# ---------------------------------------------------------------------------
# Constants / fixed parameters (v0.2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Re-derive v0.2 linear k_uptake / k_deg fits (Step 1 + Step 2A) at 34 uM
# (fit in the linear regime: uptake = k_uptake*C_ext == MM with huge K_m)
# ---------------------------------------------------------------------------
T_FIT_H = np.array([3.53, 6.44, 24.0, 48.0])
Y_RPTEC = np.array([636.0, 1377.0, 3558.0, 2792.0])
Y_NRK   = np.array([299.0,  662.0,  831.0,  104.0])
LIN_KM  = 1e9        # effectively-linear K_m for reproducing the v0.2 fit
...
print("v0.2 calibrated (linear) parameters:")
print(f"  k_uptake_RPTEC={k_uptake_RPTEC:.4g}, k_uptake_NRK={k_uptake_NRK:.4g}, k_deg_PB={k_deg_PB:.4g}")

# v0.2 reference (linear) peak total / peak C_ly at 34 uM, for fit-preservation/comparison
...
v02_peak_total_R34 = lin_tot_R34.max()
...
fig1.suptitle("Output 1: Fit preservation at 34 uM "
              "(K_m=50 deviates -- C_ext/K_m=0.68, no longer linear)", fontsize=12)
fig1.tight_layout(rect=[0, 0, 1, 0.95])
fig1.savefig(os.path.join(OUTDIR, "v03_output1_fit_preservation.png"), dpi=150)
...
fig2.suptitle("Output 2: RPTEC/TERT1 lysosomal load vs time (per K_m, all doses)",
              fontsize=13)
fig2.tight_layout(rect=[0, 0, 1, 0.96])
fig2.savefig(os.path.join(OUTDIR, "v03_output2_lysosomal_load.png"), dpi=150)
...
ax3.set_title("Output 3: Lysosomal threshold-crossing time vs dose\n"
              "(solid = 500 nM, dashed = 1000 nM; colour = K_m)")
ax3.legend(fontsize=7, ncol=2)
ax3.grid(True, which='both', alpha=0.3)
fig3.tight_layout()
fig3.savefig(os.path.join(OUTDIR, "v03_output3_threshold_crossing.png"), dpi=150)
...
# ---------------------------------------------------------------------------
print("\n" + "=" * 92)
print("OUTPUT 5: RPTEC/TERT1 summary  (24 rows)")
print("=" * 92)
...
print("=" * 92)
print("1. 34 uM calibration vs v0.2 linear (peak total intracellular):")
print(f"   v0.2 linear reference = {v02_peak_total_R34:.1f} nM")
for K_m in KM_VALUES:
    pk = results[(K_m, 34.0)]['peak_total']
    print(f"   K_m={K_m:>4.0f}: {pk:8.1f} nM  ({(pk / v02_peak_total_R34 - 1) * 100:+.1f}% vs v0.2)")
...
print("\nFigures written to ./mm_sweep/:")
for f in ("v03_output1_fit_preservation.png", "v03_output2_lysosomal_load.png",
          "v03_output3_threshold_crossing.png", "v03_output4_peak_cly_vs_dose.png"):
    print(f"  {f}")
print("=" * 92)
