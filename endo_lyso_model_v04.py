"""
Intracellular drug accumulation model -- v0.4
Calibrated against Jarzina et al. (2022) Front. Toxicol. 4:864441, Figure 6A.

v0.4 change (per prompts/model_v04_updated_instructions.md): the fixed trafficking
rate constants are updated to directly measured, proximal-tubule-specific values
from Shipman et al. (2022), a 5-compartment ODE model of megalin trafficking in
opossum kidney (OK) cells whose compartments map cleanly onto ours (their
AEE = our EE, AV/Rab7 = our LE/MVB, Lys = our LY; their Rab11a "DAT" recycling
compartment collapses into our k_rec efflux term).

  * k_mat  (EE->LE):  0.04  -> 0.048  /min   (their k_m,1; corroborates old value to ~20%)
  * k_fuse (LE->LY):  0.02  -> 0.0094 /min   (their k_m,2; ~2x slower than assumed)
  * k_rec  (EE->surf): fixed 0.02 -> nominal 0.02 with an UNCERTAINTY BAND 0.02-0.046
        /min (upper bound = their k_DAT,f fast early-endosome recycling). Not
        hard-fixed: the headline fit uses the nominal 0.02 (continuous with v0.2,
        so the change is attributable to k_mat/k_fuse alone) and the band is
        propagated onto k_uptake.
  * k_esc  (LE->cytosol): kept 0.0002 (Gilleron) -- receptor doesn't escape, cargo
        is committed to the lysosome once it dissociates in the acidifying LE.
  * k_deg: still drug-fitted. Shipman's lysosomal rate is a megalin-protein
        property (~9-min residence); ours is the drug's nuclease/protease
        resistance -- a different quantity, so NOT transferred.

Everything else (3-compartment ODE structure, linear-regime calibration to the
34 uM polymyxin B data, log-space fitting, BDF solver, fmol/cell->nM conversion)
is unchanged from v0.2. The v0.2 files are retained intact for comparison.
...
# Fixed trafficking rate constants (/min) -- cell machinery, not drug.
# v0.4: Shipman et al. (2022) OK-cell values (Fig 4C). k_rec held at its nominal
# (lower-bound) value here; the recycling BAND is explored separately below.
K_REC_NOMINAL = 0.02             # /min  (band lower bound; = old v0.2 value)
K_REC_UPPER   = 0.046            # /min  (Shipman k_DAT,f fast EE recycling)
...
# ---------------------------------------------------------------------------
# 7. Fitting: Steps 1, 2A, 2B (headline fit at nominal k_rec)
# ---------------------------------------------------------------------------
print("=" * 72)
print("FITTING (v0.4 -- Shipman trafficking rates, nominal k_rec = %.3g)" % K_REC_NOMINAL)
print("=" * 72)
...
# ---------------------------------------------------------------------------
# 7b. k_rec uncertainty band -> band on fitted uptake (Step 1 & 2A refit at
#     the band endpoints). k_rec is NOT hard-fixed; this propagates the
#     0.02-0.046 /min Shipman recycling range onto k_uptake.
# ---------------------------------------------------------------------------
print("\n" + "-" * 72)
print("k_rec UNCERTAINTY BAND (refit Steps 1 & 2A at band endpoints)")
print("-" * 72)
band = {}
for kr in (K_REC_NOMINAL, K_REC_UPPER):
    fx = dict(FIXED_DEFAULT); fx['k_rec'] = kr
    b1, _, _ = fit(['k_uptake', 'k_deg'], KUPTAKE_INIT, KDEG_INIT,
                   V_CELL_RPTEC, T_FIT_H, y_RPTEC, fixed=fx)
    b2, _, _ = fit(['k_uptake'], KUPTAKE_INIT, b1['k_deg'],
                   V_CELL_NRK, T_FIT_H, y_NRK, fixed=fx)
    band[kr] = dict(k_uptake_R=b1['k_uptake'], k_deg=b1['k_deg'],
                    k_uptake_N=b2['k_uptake'], ratio=b1['k_uptake'] / b2['k_uptake'])
    print(f"  k_rec={kr:.3f}: k_uptake_R={b1['k_uptake']:.4g}, k_deg={b1['k_deg']:.4g}, "
          f"k_uptake_N={b2['k_uptake']:.4g}, ratio={b1['k_uptake'] / b2['k_uptake']:.2f}")
kuR_lo, kuR_hi = band[K_REC_NOMINAL]['k_uptake_R'], band[K_REC_UPPER]['k_uptake_R']
print(f"  -> k_uptake_RPTEC band: {kuR_lo:.4g} to {kuR_hi:.4g} fmol/cell/min/uM "
      f"({(kuR_hi / kuR_lo - 1) * 100:+.0f}% at upper k_rec)")


# ---------------------------------------------------------------------------
# 8. Stress-test of fixed trafficking parameters (grids RE-CENTERED on v0.4)
...
# ---------------------------------------------------------------------------
STRESS = {
    'k_mat':  [0.0048, 0.012, 0.048, 0.12, 0.48],       # centered on v0.4 default 0.048
    'k_rec':  [0.002, 0.005, 0.02, 0.05, 0.2],          # centered on nominal 0.02
    'k_fuse': [0.00094, 0.0024, 0.0094, 0.024, 0.094],  # centered on v0.4 default 0.0094
    'k_esc':  [0.00002, 0.00005, 0.0002, 0.0005, 0.002],# centered on 0.0002 (unchanged)
}
stress_results = {}
print("\n" + "=" * 72)
...
print("\n" + "=" * 72)
print("OUTPUT 3: PARAMETER SUMMARY (v0.4)")
print("=" * 72)
rows = [
    ("k_mat (fixed, Shipman k_m,1)",           f"{FIXED_DEFAULT['k_mat']:.4g} /min"),
    ("k_rec (nominal; band 0.02-0.046)",       f"{FIXED_DEFAULT['k_rec']:.4g} /min"),
    ("k_fuse (fixed, Shipman k_m,2)",          f"{FIXED_DEFAULT['k_fuse']:.4g} /min"),
    ("k_esc (fixed, Gilleron)",                f"{FIXED_DEFAULT['k_esc']:.4g} /min"),
    ("k_uptake_RPTEC (Step 1, nom k_rec)",     f"{k_uptake_RPTEC:.4g} fmol/cell/min/uM"),
    ("k_uptake_RPTEC band (k_rec 0.02-0.046)", f"{kuR_lo:.4g} - {kuR_hi:.4g}"),
