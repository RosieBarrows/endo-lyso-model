"""
Shared core for the endo-lysosomal drug accumulation model.

Consolidates the ODE system, simulation, and fitted-parameter values that were
previously duplicated (with slight drift) across endo_lyso_model_v02.py and
concentration_sweep_mm.py. Used by the analysis scripts and by app.py
(the Streamlit explorer).

Model structure and calibration are unchanged from v0.2 (linear-regime fit to
Jarzina et al. 2022 Fig 6A, RPTEC/TERT1 + NRK-52E, polymyxin B) with the v0.3
Michaelis-Menten uptake term (model_v03_michaelis_menten_instructions.md):

    dC_ee/dt = V_max*C_ext/(K_m+C_ext) - (k_mat + k_rec) * C_ee
    dC_le/dt = k_mat * C_ee            - (k_fuse + k_esc) * C_le
    dC_ly/dt = k_fuse * C_le           - k_deg * C_ly

V_max = k_uptake_fitted * K_m, so at C_ext << K_m the model reduces to the
v0.2 linear fit (uptake = k_uptake * C_ext); at C_ext >> K_m uptake saturates.
"""
...
# ---------------------------------------------------------------------------
# Fixed trafficking parameters (cell machinery, not drug) -- v0.1/v0.2 defaults
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Frozen v0.2 fitted parameters (linear-regime k_uptake, k_deg), from
# endo_lyso_model_v02.py Steps 1 (RPTEC) and 2A (NRK, k_deg fixed).
# Recomputing these live is unnecessary -- the fit is deterministic given the
# fixed digitised data, so we hardcode the result and let the app vary the
# *mechanistic* parameters (K_m, dose, trafficking rates) around them.
# ---------------------------------------------------------------------------
FITTED = {
    "k_uptake_RPTEC": 2.7717e-07,  # fmol/cell/min/uM  (Step 1)
    "k_uptake_NRK":   3.5489e-08,  # fmol/cell/min/uM  (Step 2A, k_deg fixed)
    "k_deg_PB":       2.3992e-04,  # /min  (Step 1, shared with Step 2A)
    "k_deg_NRK_free": 1.5941e-03,  # /min  (Step 2B, NRK k_deg free -- reference only)
}
